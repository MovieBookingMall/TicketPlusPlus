package com.lbc.ticketplus.tool;
...
public class UserCF {
    ...
    public void testGetCosineSimilarity() {
        List<String> _userItems = new ArrayList<String>();
        _userItems.add("0 a b c d e g");
        _userItems.add("1 a b c e f h");
        _userItems.add("2 b c d e f g");
        _userItems.add("3 a b c");
        Map<String, Double> cosineSimilarity = getCosineSimilarity(1, _userItems);
        Set<Entry<String, Double>> entries = cosineSimilarity.entrySet();
        for (Entry e : entries) {
            System.out.println("similarity with " + e.getKey() + ": " + e.getValue());
        }

    }

    /**
     * 基于用户的协同过滤推荐算法计算余弦相似度
     *
     * @param _userItems * 用户ID	物品ID集合
     *                   A		a b d
     *                   B		a c
     *                   C		b e
     *                   D		c d e
     * @return
     */
    public static Map<String, Double> getCosineSimilarity(Integer _userId, List<String> _userItems) {
        int N = _userItems.size();
        ...
        for (int i = 0; i < N; i++) {
            String[] user_item = _userItems.get(i).split(" ");
        ...
        //计算用户之间的相似度【余弦相似性】返回结果
        Map<String, Double> result = new HashMap<>();
        int recommendUserId = userID.get(_userId.toString());
...
//                System.out.println(idUser.get(recommendUserId) + "--" + idUser.get(j) + "相似度:" + sparseMatrix[recommendUserId][j] / Math.sqrt(userItemLength.get(idUser.get(recommendUserId)) * userItemLength.get(idUser.get(j))));
                ...
                result.put(idUser.get(j), similarity);
            }
        }
        return result;
    }
...
//https://blog.csdn.net/u011514514/article/details/72722803
